...
class RadiologyQwenProjectorLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.radiology_projector_config = {
        ...
        self.radiology_projectors = nn.ModuleDict(
            {
                "radiology": ModalityProjector(
                    in_dim=self.radiology_in_dim,
                    out_dim=self.hidden_size,
                    projector_type=self.radiology_projector_config["projector_type"],
                    num_latents=self.radiology_projector_config["projector_num_latents"],
                    depth=self.radiology_projector_config["projector_depth"],
                    num_heads=self.radiology_projector_config["projector_num_heads"],
                    mlp_ratio=self.radiology_projector_config["projector_mlp_ratio"],
                    dropout=self.radiology_projector_config["projector_dropout"],
                )
            }
        )
    ...
    def move_trainable_modules_to(self, device: torch.device, *, dtype: torch.dtype | None = None) -> None:
        self.radiology_projectors.to(device=device, dtype=dtype)
    ...
    def forward(
        ...
        *,
        ...
        radiology_projected, _ = self.radiology_projectors["radiology"](radiology_features, radiology_feature_mask)
        text_embeddings = self.language_model.get_input_embeddings()(input_ids)
        radiology_projected = radiology_projected.to(dtype=text_embeddings.dtype)

        prefix_attention = self.radiology_projectors["radiology"].build_output_mask(
        ...
        return forward_language_model_with_soft_prefix(
            ...
            prefix_length=radiology_projected.shape[1],
        )
