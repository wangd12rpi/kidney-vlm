...
from kidney_vlm.radiology.series_feature_store import load_series_feature_array, looks_like_series_feature_ref
...
DEFAULT_RADIOLOGY_PROJECTOR_PROMPT_TEXTS = (
    ...
    "Write a radiology caption.",
    "State the main radiology findings.",
    "Give a concise radiology description.",
)
...
def _load_npy_feature_tensor(path: Path, max_tokens: int) -> torch.Tensor:
    features = np.load(path)
    if features.ndim == 1:
        features = features[np.newaxis, :]
    if features.ndim != 2:
        raise ValueError(f"Expected radiology embedding with 1 or 2 dimensions, got shape {tuple(features.shape)}")
    return _sample_sequence_features(torch.from_numpy(features).to(dtype=torch.float32), max_tokens=max_tokens)
...
def _load_radiology_feature_tensor(root_dir: Path, values: Any, max_slice_tokens: int) -> torch.Tensor:
    ...
    for raw_value in _normalize_list(values):
        ...
        if looks_like_series_feature_ref(text):
            features = load_series_feature_array(root_dir, text)
            tensors.append(
                _sample_sequence_features(torch.from_numpy(features).to(dtype=torch.float32), max_tokens=max_slice_tokens)
            )
            continue
        ...
        if candidate.suffix.lower() == ".npy":
            tensors.append(_load_npy_feature_tensor(candidate, max_tokens=max_slice_tokens))
            continue
...
class RadiologyProjectorQACollator:
    ...
    def _build_text_pair(self, feature: dict[str, Any]) -> tuple[list[int], list[int]]:
        answer = str(feature.get(self.answer_field, "")).strip()
        if not answer:
            raise ValueError("Empty answer/caption encountered in radiology projector batch.")
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        metadata_keys = ("sample_id", "project_id", "source", "patient_id", "study_id", "image_name")
        metadata: dict[str, list[Any]] = {key: [] for key in metadata_keys}
        ...
        batch_size = len(features)
        max_text_tokens = max(len(item) for item in text_input_ids)
        max_slice_tokens = max(tensor.shape[0] for tensor in radiology_tensors)
        radiology_dim = radiology_tensors[0].shape[1]

        input_ids = torch.full((batch_size, max_text_tokens), pad_token_id, dtype=torch.long)
        attention_mask = torch.zeros((batch_size, max_text_tokens), dtype=torch.long)
        labels = torch.full((batch_size, max_text_tokens), -100, dtype=torch.long)
        radiology_features = torch.zeros((batch_size, max_slice_tokens, radiology_dim), dtype=torch.float32)
        radiology_feature_mask = torch.zeros((batch_size, max_slice_tokens), dtype=torch.long)
