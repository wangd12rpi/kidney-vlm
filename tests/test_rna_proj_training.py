...
def _load_script_module():
    ...
    script_path = repo_root / "scripts" / "04_rna_proj" / "04_train_rna_projectors.py"
    spec = importlib.util.spec_from_file_location("train_rna_projectors_script", script_path)
...
def test_rna_run_artifacts_are_saved_inside_timestamped_run_dir() -> None:
    import torch
    ...
    cfg = OmegaConf.create(
        {
            "rna_proj": {
                ...
                "rna_prefix_tokens": 8,
                "rna_prefix_expander_mlp_ratio": 1.0,
                "max_rna_tokens": 1,
    ...
    tmp_path = artifacts_root / f"rna_proj_training_{uuid4().hex}"
    ...
    try:
        ...
        assert "rna_projector_state_dict" in state
        assert state["rna_embedding_dim"] == 512
        assert state["max_rna_tokens"] == 1
        assert run_output_dir.parent == tmp_path / "qwen3_5_9b" / "rna"
        assert run_output_dir.name.startswith("rna_mlp_")
        ...
        assert metadata["best_checkpoint_path"] == (run_output_dir / "best.ckpt").resolve().relative_to(repo_root).as_posix()
    finally:
