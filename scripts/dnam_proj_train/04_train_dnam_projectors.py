...
def _build_run_output_dir(*, output_root: Path, modality_tag: str, projector_type: str) -> Path:
...
def _save_artifacts(
    *,
    ...
    projector_type = str(cfg.dnam_proj_train.get("projector_type", "mlp")).strip() or "mlp"
    torch.save(
        {
            "dnam_projector_state_dict": model.projectors.state_dict(),
            "model_name_or_path": str(cfg.dnam_proj_train.model_name_or_path),
            "dnam_embedding_dim": int(cfg.dnam_proj_train.dnam_embedding_dim),
            "projector_type": projector_type,
            "projector_num_latents": int(cfg.dnam_proj_train.get("projector_num_latents", 64)),
            "projector_depth": int(cfg.dnam_proj_train.get("projector_depth", 2)),
            "projector_num_heads": int(cfg.dnam_proj_train.get("projector_num_heads", 8)),
            "projector_mlp_ratio": float(cfg.dnam_proj_train.get("projector_mlp_ratio", 4.0)),
            "projector_dropout": float(cfg.dnam_proj_train.get("projector_dropout", 0.0)),
            "hidden_size": int(model.hidden_size),
            "max_dnam_tokens": int(cfg.dnam_proj_train.max_dnam_tokens),
    ...
    if not config_path.exists():
        OmegaConf.save(config=cfg.dnam_proj_train, f=str(config_path))
    return state_path
...
def main() -> None:
    ...
    if not bool(stage_cfg.instantiate_model):
        print("dnam_proj_train.instantiate_model=false; nothing to do.")
        return
