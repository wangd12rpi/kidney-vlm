...
class RNAProjectorQADataset(Dataset):
...
def load_cfg():
    return load_script_cfg(
        repo_root=ROOT,
        config_relative_path="04_rna_proj/04_train_rna_projectors.yaml",
...
def _build_tokenizer(model_name_or_path: str, trust_remote_code: bool):
    try:
        ...
        raise RuntimeError("transformers is required for RNA projector training.") from exc
...
def _build_lr_scheduler(
    *,
    ...
    try:
        ...
        raise RuntimeError("transformers is required for RNA projector scheduler setup.") from exc
        ...
        for step, batch in enumerate(loop, start=1):
            ...
            with torch.autocast(device_type=device.type, dtype=autocast_dtype, enabled=use_autocast):
                outputs = model(
                    ...
                    labels=batch["labels"],
                    rna_features=batch["rna_features"],
                    rna_feature_mask=batch["rna_feature_mask"],
                )
                loss = outputs.loss
                if loss is None:
...
def _resolve_modality_tag(stage_cfg: Any) -> str:
    raw_value = str(stage_cfg.get("modality_tag", "rna")).strip().lower() or "rna"
    return "".join(character for character in raw_value if character.isalnum() or character in {"-", "_"}).strip("_-") or "rna"


def _resolve_modality_dir_name(stage_cfg: Any) -> str:
    raw_value = str(stage_cfg.get("modality_dir_name", "")).strip().lower()
    if raw_value:
        return _slugify_label(raw_value, default="rna")
    modality_tag = _resolve_modality_tag(stage_cfg)
    return _slugify_label(modality_tag, default="rna")
...
def _save_artifacts(
    *,
    ...
    projector_type = str(cfg.rna_proj.get("projector_type", "mlp")).strip() or "mlp"
    torch.save(
        {
            ...
            "projector_num_heads": int(cfg.rna_proj.get("projector_num_heads", 8)),
            "projector_mlp_ratio": float(cfg.rna_proj.get("projector_mlp_ratio", 4.0)),
            "projector_dropout": float(cfg.rna_proj.get("projector_dropout", 0.0)),
            ...
            "max_rna_tokens": int(cfg.rna_proj.max_rna_tokens),
    ...
    save_tokenizer_snapshot = bool(cfg.rna_proj.get("save_tokenizer_snapshot", False))
    ...
    if not config_path.exists():
        OmegaConf.save(config=cfg.rna_proj, f=str(config_path))
    return state_path
...
def _write_run_metadata(
    *,
    ...
    metadata = {
        ...
        "tokenizer_model_name_or_path": str(cfg.rna_proj.model_name_or_path),
        "epoch_checkpoint_paths": [_portable_path(path) for path in epoch_checkpoint_paths],
    }
    if bool(cfg.rna_proj.get("save_tokenizer_snapshot", False)):
    ...
    metadata_path = run_output_dir / "rna_projector_metadata.json"
...
def main() -> None:
    cfg = load_cfg()
    stage_cfg = cfg.rna_proj

    qa_parquet_path = _resolve_path(stage_cfg.qa_parquet_path)
    if not qa_parquet_path.exists():
        raise FileNotFoundError(f"RNA projector training parquet not found: {qa_parquet_path}")

    if not bool(stage_cfg.instantiate_model):
        print("rna_proj.instantiate_model=false; nothing to do.")
        return
    ...
    if frame.empty:
        raise RuntimeError(f"RNA projector training parquet is empty: {qa_parquet_path}")
    if "split" not in frame.columns:
        raise RuntimeError("RNA projector training parquet must include the unified registry split column.")
    ...
    if train_frame.empty:
        raise RuntimeError("No train rows found in RNA projector training parquet.")
    if validation_frame.empty:
        raise RuntimeError("No val rows found in RNA projector training parquet. Rebuild unified/projector data first.")
    ...
    collator = RNAProjectorQACollator(
        ...
        max_rna_tokens=int(stage_cfg.max_rna_tokens),
    )
    validation_collator = RNAProjectorQACollator(
        tokenizer=tokenizer,
        root_dir=ROOT,
        max_text_length=int(stage_cfg.max_text_length),
        max_rna_tokens=int(stage_cfg.max_rna_tokens),
    ...
    model = RnaQwenProjectorLM.from_pretrained(
        ...
        rna_prefix_tokens=int(stage_cfg.get("rna_prefix_tokens", 0)),
        rna_prefix_expander_mlp_ratio=float(stage_cfg.get("rna_prefix_expander_mlp_ratio", 1.0)),
        trust_remote_code=bool(stage_cfg.trust_remote_code),
    ...
    if not trainable_parameters:
        raise RuntimeError("No trainable parameters found for RNA projector stage.")
    ...
    print("Stage 1 RNA projector training")
    print(f"RNA projector parquet: {qa_parquet_path}")
    ...
    print(f"RNA prefix tokens: {int(stage_cfg.get('rna_prefix_tokens', 0))}")
    print(f"RNA prefix expander MLP ratio: {float(stage_cfg.get('rna_prefix_expander_mlp_ratio', 1.0))}")
    ...
    print("RNA projector training complete.")
