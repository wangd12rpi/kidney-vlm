...
def main() -> None:
    ...
    local_jobs = _build_local_slide_jobs(
        registry_df,
        patch_features_dir=patch_features_dir,
    ...
    _print_step_line(4, "local downloaded slides needing extraction", ("slides", len(local_jobs)))
    _print_timing_line(
        "Config",
    ...
    if not local_jobs:
        print("No local downloaded pathology slides require extraction.")
        return
    ...
    patch_count_values: list[int] = []

    slide_loop = tqdm(local_jobs, total=len(local_jobs), desc="Extracting local pathology slide embeddings")
    for job in slide_loop:
        ...
        try:
            ...
            failed += 1
            slide_loop.write(f"[error] {slide_path.name}: {exc}")
            if not skip_errors:
                raise
        finally:
        ...
        patch_count = int(_read_patch_count(coords_path, final_patch_path, save_format))
        patch_count_values.append(patch_count)
    ...
    avg_patch_count = sum(patch_count_values) / float(len(patch_count_values)) if patch_count_values else 0.0
